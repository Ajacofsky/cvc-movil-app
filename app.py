...
def procesar_panel_camara(titulo_ojo, key_suffix):
    archivo = st.camera_input(f"📷 Capturar en Vivo - {titulo_ojo}", key=f"cam_{key_suffix}")
    
    incapacidad_final, grados_finales, img_original = 0.0, 0.0, None
    t_cuad, t_circ = 0, 0
    
    if archivo is not None:
        with st.spinner("Procesando imagen (Escudo Activo)..."):
            try:
                # 1. Leer imagen de manera segura
                nparr = np.frombuffer(archivo.getvalue(), np.uint8)
                img_raw = cv2.imdecode(nparr, cv2.IMREAD_COLOR)
                
                # EVITAR ERROR SILENCIOSO SI LA CÁMARA FALLA
                if img_raw is None:
                    st.error("⚠️ La cámara no capturó la foto correctamente. Por favor, intente de nuevo.")
                    return 0.0, 0.0, None
                ...
                # Si algo se rompe, lo mostrará en un cartel rojo en lugar de quedarse congelado
                st.error(f"⚠️ Hubo un error procesando la foto. Detalles para el desarrollador: {str(e)}")
                return 0.0, 0.0, None
                
        # Esta parte siempre se dibujará si la foto se procesó sin errores catastróficos
